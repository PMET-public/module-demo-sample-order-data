...
class UpgradeData implements UpgradeDataInterface
{
    ...
    public function upgrade( ModuleDataSetupInterface $setup, ModuleContextInterface $context )
    {
        ...
        if (version_compare($context->getVersion(), '0.0.4', '<=')) {
            ...
            $sql = "select TIMESTAMPDIFF(HOUR,p.created_at, o.created_at) as hours from ".$salesOrderTable." o, ".$salesOrderTable." p where p.entity_id=3999 and o.entity_id=1";
            $result = $connection->fetchAll($sql);
            $dateDiff =  $result[0]['hours']-4;
            $sql = "update ".$salesOrderTable." set created_at = date_add(created_at, interval ".$dateDiff." HOUR), updated_at = date_add(created_at, interval ".$dateDiff." HOUR) where entity_id >= 3990 and entity_id <= 3999";
            $connection->query($sql);
            $sql = "update ".$salesGridTable." set created_at = date_add(created_at, interval ".$dateDiff." HOUR), updated_at = date_add(created_at, interval ".$dateDiff." HOUR) where entity_id >= 3990 and entity_id <= 3999";
            $connection->query($sql);
            $sql = "update ".$salesOrderItemTable." set created_at = date_add(created_at, interval ".$dateDiff." HOUR), updated_at = date_add(created_at, interval ".$dateDiff." HOUR) where order_id >= 3990 and order_id <= 3999";
            $connection->query($sql);
            //create returns
